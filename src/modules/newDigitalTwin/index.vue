...
const updateModelIcon = (index = 0) => {
  // 处理区域标签和图标
  console.log(`${store.basicInfo.rootPath}/${store.basicInfo.modelIcons.viewAll}`)
  // baseAreaIcon = basePlaneIcon(getAssetImage(`/src/assets/surface-analysis/${store.basicInfo.modelIcons.viewAll}`));
...
const createSensorIcon = (iconPath) => {
  console.log(`${store.monitoring.rootPath}/${iconPath}`)
  const img = getAssetImage(`${store.monitoring.rootPath}/${iconPath}`)
  // const img = getAssetImage(`/src/assets/surface-analysis/${iconPath}`)
...
const getAssetImage = (imgPath) => {
  ...
  // const img = new URL(`/src/assets/surface-analysis/${imgPath}`, import.meta.url).href;
  console.log(new URL(`/src/${imgPath}`, import.meta.url).href)
  // const img = new URL(`${window.location.origin}/src/${imgPath}`, import.meta.url).href;
  return `${window.location.origin}/src/${imgPath}`;
};
...
const dracoPath = import.meta.env.PROD 
  ? `${import.meta.env.VITE_APP_CONTEXT_PATH}/draco/`  // 生产环境: /model/draco/
  : '/draco/';  // 开发环境: /draco/
console.log('dracoPath', dracoPath, import.meta.env.VITE_APP_CONTEXT_PATH);
