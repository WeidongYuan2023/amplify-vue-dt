...
const getIcon = (node) => {
  ...
  switch (node.type.toLowerCase()) {
    case 'am319':
      icon = getImage(`devices/am319.svg`);
      break;
    case 'ct310':
      icon = getImage(`devices/ct310.svg`);
      break;
    case 'em500_pp':
      icon = getImage(`devices/em500pp.svg`);
      break;
    case 'gs301':
      icon = getImage(`devices/gs301.svg`);
      break;
    case 'ws303':
      icon = getImage(`devices/ws303.svg`);
      break;
    default:
      icon = ' ';
  }
...
const getDevices = (node) => {
  ...
  switch (node.type.toLowerCase()) {
    case 'am319':
      icon = getImage(`devices/am319-${status}.png`);
      break;
    case 'ct310':
      icon = getImage(`devices/ct310-${status}.png`);
      break;
    case 'em500_pp':
      icon = getImage(`devices/em500pp-${status}.png`);
      break;
    case 'gs301':
      icon = getImage(`devices/gs301-${status}.png`);
      break;
    case 'ws303':
      icon = getImage(`devices/ws303-${status}.png`);
      break;
    default:
      icon = ' ';
  }
...
const getImage = (imgPath) => {
  return new URL(`/src/${store.monitoring.rootPath}/${imgPath}`, import.meta.url).href;
};
...
const getData = (sensors) => {
  return sensors
    ...
    .map((d) => {
      ...
      //   c.icon = getSvg(d, c);
      // });
      // 0:active,1:suspended,2:pending,3:deactivated
      // status: '0', ('3', '2'), '1' => 'normal', 'warning', 'error'
      // d.online = d.children.filter((c) => revertType(c.status) === 'normal').length;
      // d.outline = d.children.filter((c) => revertType(c.status) === 'warning').length;
      // d.fault = d.children.filter((c) => revertType(c.status) === 'error').length;
